 ...
 * 默认语言
 */
export const DEFAULT_LANGUAGE = SUPPORTED_LANGUAGES[0]; // auto
 ...
 * 获取 STT 语言代码
 */
export function getSTTLanguageCode(languageId) {
 ...
 * 获取 TTS 语言代码
 */
export function getTTSLanguageCode(languageId) {
  const lang = SUPPORTED_LANGUAGES.find(l => l.id === languageId);
 ...
 */
export function getAdaptiveTTSLanguage(detectedLanguage) {
 ...
 */
export function saveLanguageSetting(languageId) {
  try {
    localStorage.setItem('claude-voice-language', languageId);
    return true;
  } catch (e) {
    console.warn('[Language] 保存语言设置失败:', e);
    return false;
  }
}
...
export function loadLanguageSetting() {
  try {
    ...
    console.warn('[Language] 加载语言设置失败:', e);
  }
 ...
 * 获取常用语言的语音推荐
 */
export function getRecommendedVoices(languageId) {
  // Kokoro 推荐声音
  const kokoroVoices = {
