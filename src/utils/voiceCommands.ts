...
// Extend VoiceCommand with match info
interface MatchedCommand extends VoiceCommand {
  matchedText: string;
...
type CommandCategory = 'conversation' | 'session' | 'navigation' | 'voice' | 'model' | 'help' | 'stats' | 'tts';
...
// 匹配语音命令
export function matchVoiceCommand(text: string): MatchedCommand | null {
...
// 从文本中提取命令后的参数
export function extractCommandArgs(text: string, command: VoiceCommand): string | null {
...
// 判断文本是否可能是命令 (用于提前检测)
export function isPotentialCommand(text: string): boolean {
...
export function formatCommandHelp(): string {
  const categories: Record<CommandCategory, VoiceCommand[]> = {} as Record<CommandCategory, VoiceCommand[]>;
  ...
  for (const [category, commands] of Object.entries(categories)) {
    helpText += `**${categoryLabels[category as CommandCategory] || category}:**\n`;
